...
export const checkinHandlers = [
  // ── GET /api/eventos/:id/regras-checkin ── (spec do desafio - PDF)
  http.get('/api/eventos/:id/regras-checkin', async ({ params }) => {
    await delay(300);
    const rules = checkinRulesDB[params.id as string] || [];
    return HttpResponse.json(rules);
  }),

  // ── PUT /api/eventos/:id/regras-checkin ── (spec do desafio - PDF)
  http.put('/api/eventos/:id/regras-checkin', async ({ params, request }) => {
    await delay(400);
    const eventId = params.id as string;
    const body = (await request.json()) as CheckinRule[];
    if (!Array.isArray(body)) {
      return HttpResponse.json({ error: 'Formato inválido' }, { status: 400 });
    }
